...
class MultilingualBehavior extends CActiveRecordBehavior
{
    ...
    private function createLocalizedRelation($owner, $lang)
    {
        ...
        $owner->getMetaData()->relations[$this->localizedRelation] = new $class(
    ...
    public function afterFind($event)
    {
        ...
        if ($owner->hasRelated($this->localizedRelation)) {
    ...
    {
        try { return parent::__get($name); } catch (CException $e) {
    ...
    {
        try { parent::__set($name, $value); } catch (CException $e) {
    ...
    public function languageLabels($labels)
    {
        $res = array();
        foreach ($this->localizedAttributes as $field) {
            ...
            if (count($this->languagesAssoc)) {
                foreach ($this->languagesAssoc as $lang=>$name) {
                    if ($lang != $this->defaultLanguage) {
                        $res[$field.'_'.$lang] = $text.' ('.$name.')';
                    } else {
                        $res[$field] = $text;
                    }
                }
